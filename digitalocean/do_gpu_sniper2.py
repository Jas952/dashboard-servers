...
POLL_OFFSET   = 0.6   # half-cycle offset: interleaves with bot #1 (offset 0.0)
...
# #2 rests at :15 (SLEEP_PHASE=900); #1 rests at :00 — one bot always awake.
SLEEP_EVERY    = 1800  # rest cycle length (s) — every 30 min
SLEEP_DURATION = 60    # rest duration (s) — 1 min
SLEEP_PHASE    = 900   # offset of the rest window within the cycle
...
ACTIVE_POLL       = 1.5  # poll interval while waiting for active (faster loss detect)
...
PARALLEL_REGIONS = 8   # expanded to 8 parallel regions for better retry chances
    ...
    for tryno in range(1, PROVISION_RETRIES + 1):
        ...
        if not did:
            ...
            is_network_err = any(x in err_low for x in (
                ...
                "remotedisconnected", "remote end closed", "timed out",
